...
The 16-byte R/W scratch at `0x1200_0000` exists solely for `test_inline_patch.S` subtest 11, but it's not gated on `cfg(nh_guest_test)`. In production a read/write there round-trips real storage instead of Einstein's "unknown bank #5" zero/absorb semantics — a (small) divergence from the oracle inside an address window the comment itself says "Real Newton hardware doesn't expose anything in". Cheap to gate; gating also removes a `static mut` from production builds.
