//! BCM2835 SDHOST controller driver — PIO-only copy for the bootloader.
//!
//! This is the polled subset of the hypervisor's
//! `src/host/sd/sdhost.rs` (init, CMD17 single-block read, CMD24
//! single-block write). The DMA half of that driver — CMD25 multi-block
//! writes fed by DMA channel 6 and the background autosave machinery —
//! is deliberately not shared: it is the part under active debugging,
//! and the bootloader must keep working while it changes. The two PIO
//! paths are kept in step by hand; when a fix lands in the hypervisor's
//! `init`/`read_block`/`write_block`/command path, port it here (and
//! vice versa). The register constants in `regs.rs` *are* shared
//! (`#[path]` in `sd/mod.rs`).
//!
...
/// State the driver needs to keep between calls. (The hypervisor's
/// copy also keeps the RCA for its DMA paths; nothing here re-addresses
/// the card after CMD7.)
pub struct SdHost {
...
impl SdHost {
    ...
    pub fn init() -> Result<Self, CmdError> {
        ...
        if num_blocks == u32::MAX {
            crate::println!("sd: capacity unknown (CSD undecoded)");
        } else {
            // num_blocks * 512 / 1 MiB, computed without overflowing u32.
            crate::println!("sd: capacity {} MiB ({} sectors)", num_blocks / 2048, num_blocks);
        }
...
fn read_reg(off: usize) -> u32 {
    // SAFETY: SDHOST MMIO at fixed BCM2710 base. The MMU is off, so
    // the access is Device-nGnRnE without any mapping of our own.
...
fn send_cmd_kind(cmd: u8, arg: u32, kind: ResponseKind, dir: CmdDir) -> Result<u32, CmdError> {
    ...
    // Set BUSYWAIT for the R1b commands: CMD7 (SELECT_CARD) during
    // init, and CMD12 (STOP_TRANSMISSION) kept for parity with the
    // hypervisor driver even though this copy issues no multi-block
    // commands. The controller waits out the card's BUSY assertion
    // internally, so we never poll the DAT0 BUSY line ourselves.
    ...
    // card-internal program time; nothing else needs the CPU
    // meanwhile.
...
fn program_sdcdiv(core_clock: u32, target_hz: u32) {
    let div = core_clock.div_ceil(target_hz).saturating_sub(2);
