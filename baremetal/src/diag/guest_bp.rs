...
//!
//! `src/hv/snapshot.rs::maybe_autosave` calls `any_installed()` and
...
static GUEST_BP_FORCE_KEEP: (
    ...
    extern "C" fn(u32) -> i32,
    extern "C" fn(),
    extern "C" fn(u32, &crate::arch::trap_context::TrapContext),
...
#[no_mangle]
#[inline(never)]
pub extern "C" fn bp_hit_anchor(faulting_pc: u32, ctx: &crate::arch::trap_context::TrapContext) {
...
pub fn handle_user_bp_und(
    ...
    kprintln!(
        ...
        crate::arch::banked::sp_for_mode(ctx, cpsr), cpsr & 0x1F
    );
    kprintln!(
        "    r14 = {:#010x}  (LR of mode {:#x} via Table D1-79)",
        crate::arch::banked::lr_for_mode(ctx, cpsr), cpsr & 0x1F
    );
