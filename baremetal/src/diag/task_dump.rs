...
pub(crate) fn fmt_pc_name(pc: u32) -> ([u8; 96], usize) {
    ...
    fn render_rom_pc(pc: u32, buf: &mut [u8; 96], n: &mut usize) {
        match crate::diag::symbols::fn_name_for_pc(pc) {
            Some((entry, name)) => {
...
pub fn dump_save_area(label: &str, task_va: u32) {
    ...
    if sp_usr != 0 && sp_usr != u32::MAX {
        ...
        crate::hv::guest_mem::dump_stage1_walk(sp_usr);
    }
}
...
pub fn periodic(ctx: &crate::arch::trap_context::TrapContext) -> bool {
...
#[no_mangle]
#[inline(never)]
pub extern "C" fn dump_current_chain(ctx: &crate::arch::trap_context::TrapContext) {
    let elr: u64;
...
#[no_mangle]
#[inline(never)]
pub extern "C" fn dump_chain_at(ctx: &crate::arch::trap_context::TrapContext, pc: u32) {
    let spsr: u64;
    // SAFETY: reading sysregs has no side effects.
    unsafe {
        core::arch::asm!(
    ...
    let mode = spsr & 0x1F;
    let sp = crate::arch::banked::sp_for_mode(ctx, spsr);
    let lr = crate::arch::banked::lr_for_mode(ctx, spsr);
