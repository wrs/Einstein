...
//! it uses come from `crate::aarch32_emit` (compile-time-verified).
//!
...
use crate::guest_mem::{rom_host_pa, write_rom_code_word, write_rom_data_word};
...
pub unsafe fn install_dabt_fast_trampoline(rom_ptr: *mut u32) {
    unsafe {
        ...
        let beq = |from: usize, to: usize| -> u32 {
            crate::aarch32_emit::b_cond(pc_of(from), pc_of(to), crate::aarch32_emit::COND_EQ)
        };
        ...
        let b_far = |from_byte_offset: u32, to_byte_offset: u32| -> u32 {
            crate::aarch32_emit::b(from_byte_offset, to_byte_offset)
        };
        // `ldr r0, [pc, #imm12]` literal-load. `from`/`to` are word slot
        // indices; the literal lives at slot `to`.
        let ldr_r0_lit =
            |from: usize, to: usize| -> u32 { crate::aarch32_emit::ldr_rd_lit(pc_of(from), 0, pc_of(to)) };
