...
fn handle_hvc(ctx: &mut TrapContext, iss: u32) {
    ...
    match imm {
        ...
        v if v == crate::rom_patches::THROW_REF_EXCEPTION_PROBE_HVC_IMM => {
            handle_throw_ref_exception_probe(ctx);
        }
...
fn handle_und(ctx: &mut TrapContext) {
    ...
    match insn {
        ...
        _ if insn == rom_patches_hvc_insn(crate::rom_patches::THROW_REF_EXCEPTION_PROBE_HVC_IMM) => {
            handle_throw_ref_exception_probe_with(ctx, spsr_und as u32);
            return_to_guest_from_und(ctx, (faulting_pc + 4) as u64, spsr_und);
            return;
        }
...
/// Iter-43: probe at `Throw` entry (0x000B_00C8). Logs every kernel
/// exception throw with (name string, r0..r2, caller_lr, sp). This
/// gives the throw-site → exception-name mapping in one shot, so we
/// don't have to probe 20+ candidate `bl Throw` sites individually.
fn handle_throw_entry_probe(ctx: &mut TrapContext) {
    let spsr_el2 = read_sysreg!("spsr_el2") as u32;
    handle_throw_entry_probe_with(ctx, probe_source_cpsr(spsr_el2));
}

fn handle_throw_entry_probe_with(ctx: &mut TrapContext, source_cpsr: u32) {
    static SEQ: AtomicU32 = AtomicU32::new(0);
    let seq = SEQ.fetch_add(1, Ordering::Relaxed);
    let r0 = ctx.x[0] as u32;
    let r1 = ctx.x[1] as u32;
    ...
    // Read the exception-name C-string pointed to by r0.
    let (buf, len) = read_cstr_at(r0, 64);
    let name = core::str::from_utf8(&buf[..len]).unwrap_or("<non-utf8>");

    kprintln!(
        "Throw #{}: name={:?} (r0={:#010x}) r1={:#010x} r2={:#010x} caller_lr={:#010x} sp={:#010x} mode={:#x}",
        seq, name, r0, r1, r2, lr, sp, mode,
    );
...
fn handle_throw_ex_intrp_probe_with(ctx: &mut TrapContext, source_cpsr: u32) {
    static SEQ: AtomicU32 = AtomicU32::new(0);
    let seq = SEQ.fetch_add(1, Ordering::Relaxed);
    let r0 = ctx.x[0] as u32;
    let r1 = ctx.x[1] as u32;
...
fn handle_throw_ref_exception_probe(ctx: &mut TrapContext) {
    let spsr_el2 = read_sysreg!("spsr_el2") as u32;
    handle_throw_ref_exception_probe_with(ctx, probe_source_cpsr(spsr_el2));
}

/// Probe at `ThrowRefException(char* name, RefVar const& value)` entry
/// (ROM 0x2f5730). Captures the caller LR — i.e. the NewtonScript-
/// runtime PC that constructed the type-mismatch throw — which the
/// generic `Throw` probe can't see (it only fires from inside
/// ThrowRefException's own `bl Throw` at 0x2f57f8). Pairs the name
/// string and the RefVar pointer with the caller-PC bisect.
fn handle_throw_ref_exception_probe_with(ctx: &mut TrapContext, source_cpsr: u32) {
    static SEQ: AtomicU32 = AtomicU32::new(0);
    let seq = SEQ.fetch_add(1, Ordering::Relaxed);
    let r0 = ctx.x[0] as u32;
    let r1 = ctx.x[1] as u32;
...
/// Iter-48: probe at `TFlashStore::Lookup` (PC=0x000c_74cc,
/// `ldr r0, [r0, r1, lsl #2]`) — the table-indexed load. r0 here
/// is the table base (= 0x0c605848 per iter-47); r1 is the
/// computed index (= r7 >> [r4+96]). Captures (base, index, entry)
/// and halts when `table[index]` is outside RAM/ROM (= wild).
fn handle_lookup_table_idx_probe(ctx: &mut TrapContext) {
    let spsr_el2 = read_sysreg!("spsr_el2") as u32;
    handle_lookup_table_idx_probe_with(ctx, probe_source_cpsr(spsr_el2));
}

fn handle_lookup_table_idx_probe_with(ctx: &mut TrapContext, source_cpsr: u32) {
    static SEQ: AtomicU32 = AtomicU32::new(0);
    let seq = SEQ.fetch_add(1, Ordering::Relaxed);

    let base = ctx.x[0] as u32;
    let index = ctx.x[1] as u32;
    let r4 = ctx.x[4] as u32;
    let r7 = ctx.x[7] as u32;
    let lr = crate::banked::lr_for_mode(ctx, source_cpsr);
    let sp = crate::banked::sp_for_mode(ctx, source_cpsr);
    ...
    let entry_va = base.wrapping_add(index.wrapping_shl(2));
    let entry = guest_mem::read_word_va(entry_va).unwrap_or_else(|| {
        kprintln!(
            "  ★ Lookup-idx #{} load from {:#010x} returned None — would have faulted",
            seq, entry_va,
        );
        0xDEADBEEF
    });

    // "Sane" iff in RAM (0x0c000000..0x10000000) or ROM
    // (0..0x800000), or NULL (legitimate "no entry"). NULL is
    // logged but not halted because PhysBlock(NULL) is caught
    // upstream by the iter-44/iter-46 probe.
    let in_ram = (0x0c00_0000..0x1000_0000).contains(&entry);
    let in_rom = entry < 0x0080_0000;
    let entry_sane = entry == 0 || in_ram || in_rom;

    if !entry_sane {
        halt_invariant("TFlashStore::Lookup loaded WILD entry from table[index]", || {
            kprintln!(
                "  base={:#010x}  index={:#x} ({})  entry_va={:#010x}  entry={:#010x}  WILD",
                base, index, index, entry_va, entry,
            );
            kprintln!(
                "  TFlashStore* r4={:#010x}  r7 (search key)={:#010x}",
                r4, r7,
            );
            kprintln!(
                "  caller_lr={:#010x}  sp={:#010x}  fp={:#010x}  src_mode={} ({:#x})  seq=#{}",
                lr, sp, fp, describe_aarch32_mode(mode), mode, seq,
            );
            kprintln!();
            kprintln!("  Table neighbourhood @{:#010x} (32 entries around index {:#x}):", base, index);
            let start_idx = index.saturating_sub(8);
            for i in 0..32u32 {
                let idx = start_idx.wrapping_add(i);
                let va = base.wrapping_add(idx.wrapping_shl(2));
                let marker = if idx == index { " <-- BAD" } else { "" };
                match guest_mem::read_word_va(va) {
                    Some(w) => kprintln!("    table[{:#04x}] @{:#010x} = {:#010x}{}", idx, va, w, marker),
                    None    => kprintln!("    table[{:#04x}] @{:#010x} = (unmapped){}", idx, va, marker),
                }
            }
            kprintln!();
            kprintln!("  TFlashStore dump @{:#010x} (16 words):", r4);
            for i in 0..16u32 {
                let off = i * 4;
                let va = r4.wrapping_add(off);
                match guest_mem::read_word_va(va) {
    ...
    if seq < 8 || seq % 64 == 0 {
        kprintln!(
            "Lookup-idx #{}: base={:#010x} idx={:#x} entry={:#010x} r7={:#010x} caller_lr={:#010x}",
            seq, base, index, entry, r7, lr,
        );
    }
...
fn handle_idleproc_probe_with(ctx: &mut TrapContext, source_cpsr: u32) {
    ...
    if budget > 0 {
        ...
        // Resolve VA→PA for the CList header AND for the suspect
        // third aliasing VA (0x0c320000 — discovered via per-IRQ
        // snapshot in iter 3 by reverse-engineering the corrupting
        // SetFreeChain APCS push at sp=0x0c3207dc). If both translate
        // to the same PA, the alias is confirmed.
        let clist_pa = guest_mem::translate_va(clist).unwrap_or(0xDEAD_DEA8);
        let alias_pa = guest_mem::translate_va(0x0c32_07dc).unwrap_or(0xDEAD_DEA9);
        kprintln!(
            "IdleProc #{:03} ENTER this={:#010x} inner={:#010x} clist={:#010x} (PA={:#010x}) alias_test_VA=0x0c3207dc->PA={:#010x} count={} esize={} ebase={:#010x} src_mode={:#x} sp={:#010x}",
            n, this, inner, clist, clist_pa, alias_pa, count, esize, ebase, source_cpsr & 0x1F, sp,
        );
