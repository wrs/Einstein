...
/// File handle returned by `SYS_OPEN(":tt", "w")`. `-1` sentinel means
/// "not opened yet" — `Writer` falls back to per-byte SYS_WRITEC in
/// that case (covers any kprintln issued before `uart::init()`).
static STDOUT_FH: AtomicI64 = AtomicI64::new(-1);
...
///
/// SAFETY: HLT #0xF000 is the AArch64 semihosting trap. QEMU's and the
/// FVP AEM model's handlers intercept it and return to EL2 without
...
/// Push a byte buffer to the host stdout via SYS_WRITE. The handle
/// must already be open (callers gate on `STDOUT_FH >= 0`). Short
/// writes are silently ignored — we'd have nowhere to surface the
/// error anyway, and dropping a few bytes of console output is a
/// better failure mode than recursing back into the same write path.
fn sh_write_bytes(fh: i64, data: &[u8]) {
    if data.is_empty() {
        return;
    }
...
/// Per-byte SYS_WRITEC fallback for kprintlns issued before `init()`
/// completes (i.e., before STDOUT_FH is set). One HLT per character —
/// slow but bounded to the handful of `print_banner` / `print_caps`
/// lines that run before `uart::init`.
fn sh_writec(b: u8) {
    let byte: u8 = b;
...
/// Write a string to the host stdout via semihosting.
pub fn write_str(s: &str) {
    let fh = STDOUT_FH.load(Ordering::Acquire);
    if fh >= 0 {
        sh_write_bytes(fh, s.as_bytes());
    } else {
        for &b in s.as_bytes() {
            sh_writec(b);
        }
