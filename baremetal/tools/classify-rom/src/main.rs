...
fn walk(words: &[u32], initial_roots: &[u32]) -> (Bitmap, WalkStats) {
    let mut stats = WalkStats::default();
    let mut reach = Bitmap::new();
    let mut worklist: Vec<u32> = initial_roots.to_vec();
    stats.initial_roots = initial_roots.len();
    ...
    loop {
        ...
        // Pass A — indirect-target recovery via data words that hold a
        // pointer to a prologue-shaped target (vtables, dispatch tables,
        // callback arrays).
        for (i, &v) in words.iter().enumerate() {
            let addr = (i as u32) * 4;
            if reach.get_word(addr) { continue; }
            if v & 3 != 0 { continue; }
            if (v as usize) >= ROM_SIZE_BYTES { continue; }
            let tgt_idx = (v >> 2) as usize;
            let tgt_word = words[tgt_idx];
            if (tgt_word >> 28) == 0xF { continue; }
            if !is_known_function_start(tgt_word) { continue; }
            if reach.get_word(v) { continue; }
            worklist.push(v);
            new_roots += 1;
        }
        // Pass B — prologue sweep. Any unreached word that is itself a
        // prologue-shaped instruction is a probable function entry that
        // no direct-BL / vtable pointer happened to lead us to. Covers
        // the REX region (no symbols) and cold code reachable only via
        // register-computed calls. Data-false-positives are bounded by
        // the prologue allowlist and by the NV-cond termination inside
        // the walker; the oracle ⊆ static invariant catches the opposite
        // failure (walker missed a real instruction).
        for (i, &w) in words.iter().enumerate() {
            let addr = (i as u32) * 4;
            if reach.get_word(addr) { continue; }
            if (w >> 28) == 0xF { continue; }
            if !is_known_function_start(w) { continue; }
            worklist.push(addr);
            new_roots += 1;
        }
...
fn run(args: Args) -> Result<(), String> {
    ...
    let (reach, stats) = walk(&words, &symbols);
    ...
    writeln!(f, "  walker:").ok();
    writeln!(f, "    symbol roots: {}", stats.initial_roots).ok();
    writeln!(f, "    words walked (with revisits): {}", stats.words_walked).ok();
    writeln!(f, "    NV-cond words skipped: {}", stats.nv_cond_skips).ok();
    writeln!(f, "    indirect-target recovery passes: {}", stats.indirect_passes).ok();
    writeln!(f, "    indirect-target roots added: {}", stats.indirect_roots_added).ok();
